...
class ReadGeoGraph():
    ...
    def form_geo_datalist(self, num_graph, graph_feature_list, graph_label_list, edge_index, internal_edge_index):
        geo_datalist = []
        for i in range(num_graph):
            ...
            geo_data = Data(x=graph_feature, edge_index=edge_index, internal_edge_index=internal_edge_index, label=graph_label)
            geo_datalist.append(geo_data)
        return geo_datalist


def read_batch(index, upper_index, x_input, y_input, num_feature, num_node, edge_index, internal_edge_index, graph_output_folder):
    # FORMING BATCH FILES
    ...
    print(xBatch.shape)
    print(yBatch.shape)
    # PREPARE LOADING LISTS OF [features, labels, edge_index, internal_edge_index]
    ...
    geo_datalist = ReadGeoGraph().form_geo_datalist(num_graph, graph_feature_list, graph_label_list, edge_index, internal_edge_index)
    return geo_datalist
